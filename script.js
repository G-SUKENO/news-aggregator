// =================================================================
// script.js 完全版 (生成時刻表示、JST強制表示、新JSON形式対応)
// =================================================================
 ...
 * news.jsonの時刻はタイムゾーン情報（+09:00）を含むため、Dateオブジェクトは正確な時刻を内部保持する。
...
function createNewsListItem(article, companyName, newLabel = '') {
    ...
    // ★修正点: タイムゾーンを明示的に 'Asia/Tokyo' に指定し、JSTで表示する ★
...
function searchNews() {
    ...
    // 検索結果を表示するため、アコーディオンを開く処理
    document.querySelectorAll('.archive-item').forEach(details => {
        const hasVisibleItem = details.querySelector('.archive-content > .news-item:not(.hidden-by-search)');
        if (hasVisibleItem) {
            details.open = true;
